...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a leakage-safe Baseball moneyline dataset"
    )
    parser.add_argument(
        "--observations",
        type=Path,
        default=Path("data/baseball/market_observations.jsonl"),
    )
    parser.add_argument(
        "--results", type=Path, required=True, help="JSONL of resolved game records"
    )
    parser.add_argument(
        "--output",
        type=Path,
