...
def main() -> None:
    ...
    series: dict[tuple[str, str, str, str, str], list[tuple[datetime, float]]] = (
        defaultdict(list)
    )
    for r in rows:
        ...
        movements.append(
            {
                ...
                "first_at": first_t.isoformat(),
                "last_at": last_t.isoformat(),
            }
        )
    ...
    for r in rows:
        ...
        if k and t and v:
            ...
            if 0 <= mins <= 60:
                rr = dict(r)
                rr["_mins_to_kickoff"] = mins
                late.append(rr)
    late_groups: dict[tuple[str, str, str, str], list[dict[str, Any]]] = defaultdict(
        list
    )
    for r in late:
        late_groups[
    ...
    family_counts = {
        fam: sum(
            c for m, c in market_counts.items() if any(t in m.casefold() for t in toks)
        )
    ...
    result = {
        ...
        "dataset": {
            ...
            "bookmakers": len(book_counts),
            "markets": len(market_counts),
        },
        "excluded_from_model": {
            "player_props": True,
            "market_tokens": list(EXCLUDED_MARKET_TOKENS),
        },
        ...
        "event_density": {
            "median_observations_per_event": statistics.median(event_counts.values())
            if event_counts
            else 0,
            "max": max(event_counts.values()) if event_counts else 0,
            "min": min(event_counts.values()) if event_counts else 0,
        },
        "cross_bookmaker_dispersion": {
            "groups": len(dispersion),
            "largest_ranges": dispersion[:25],
        },
        "price_movement": {
            "series": len(movements),
            "largest_absolute_moves": movers[:40],
        },
        ...
        json.dumps(
            {
                ...
                "movement_series": len(movements),
                "closing_rows": len(late),
                "player_props_excluded": True,
            }
        )
    )
