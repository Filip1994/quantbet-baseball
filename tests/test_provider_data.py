...
def test_standings_same_team_can_exist_in_multiple_provider_groups() -> None:
    base = {
        "stage": "MLB",
        "games": {
            "played": 160,
    ...
    assert records[0].team_id == records[1].team_id == 22
    assert records[0].snapshot_id != records[1].snapshot_id
    assert {record.group_name for record in records} == {
        "American League",
        "AL East",
    }
