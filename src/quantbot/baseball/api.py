...
class BaseballAPIClient:
    ...
    def get(self, endpoint: str, params: dict[str, Any] | None = None, *, ttl_seconds: int = 0) -> list[dict[str, Any]]:
        ...
        if self.request_count >= self.settings.api_request_budget:
            raise BaseballAPIBudgetExceeded(f"Baseball API budget exhausted at {self.settings.api_request_budget} requests")
        ...
        for attempt in range(self.settings.api_max_attempts):
            if self.request_count >= self.settings.api_request_budget:
                raise BaseballAPIBudgetExceeded(f"Baseball API budget exhausted at {self.settings.api_request_budget} requests")
            request = Request(url, headers={"x-apisports-key": self.settings.api_key, "Accept": "application/json"}, method="GET")
            ...
            try:
                ...
                if retryable and attempt + 1 < self.settings.api_max_attempts:
                    self._retry_delay(attempt, (exc.headers or {}).get("Retry-After"))
                    continue
    ...
    def team_statistics(self, team_id: int, league_id: int, season: int) -> list[dict[str, Any]]:
        return self.get("teams/statistics", {"team": team_id, "league": league_id, "season": season}, ttl_seconds=86_400)
