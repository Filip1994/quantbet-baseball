...
class MarketEpoch:
    ...
    def observations_for(
        self, selection: str, bookmaker: str | None = None
...
def reconstruct_epochs(
    ...
    unique = sorted(
        by_id.values(), key=lambda o: (_instant(o.observed_at), o.observation_id)
    )
    ...
    for record in unique:
        ...
        if (
            prior is not None
            and prior.observed_at == record.observed_at
            and prior.decimal_odds != record.decimal_odds
        ):
    ...
    return tuple(
        ...
        for (family, line), values in sorted(
            grouped.items(),
...
def select_observed_price(
    epoch: MarketEpoch, selection: str, at: str
) -> OddsObservation | None:
    """Select the latest observed quote at or before ``at``; return None if absent."""
    cutoff = _instant(at)
    candidates = [
        o
        for o in epoch.observations_for(selection)
        if _instant(o.observed_at) <= cutoff
    ]
    return max(
        candidates,
        key=lambda o: (_instant(o.observed_at), o.observation_id),
        default=None,
    )
...
def opening_observation(epoch: MarketEpoch, selection: str) -> OddsObservation | None:
    return min(
        epoch.observations_for(selection),
        key=lambda o: (_instant(o.observed_at), o.observation_id),
        default=None,
    )
