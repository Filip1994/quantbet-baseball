...
def scan_once(root: Path, now: datetime | None = None) -> dict[str, int]:
    ...
    for row in predictions:
        ...
        if seconds <= 300 or seconds > 6 * 3600 or not _strong(row):
            continue
        signal_id = f"{row.get('id', row.get('game_id'))}:{now.strftime('%Y%m%d%H%M')}"
        if signal_id in existing_ids:
            continue
        signal = {"id": signal_id, "signal_source": "BASEBALL_INTRADAY_ALERT", "created_at": now.isoformat(), "status": "PAPER_SIGNAL", "decision": "UPLATI SADA", "game_id": row.get("game_id"), "league": row.get("league"), "match": row.get("match") or f"{row.get('away', '')} @ {row.get('home', '')}", "kickoff": kickoff.isoformat(), "market": row.get("market"), "market_display": row.get("market_display", row.get("market")), "odd": row.get("odd"), "bookmaker": row.get("bookmaker"), "model_probability": row.get("model_probability"), "calibrated_probability": row.get("calibrated_probability"), "probability_edge": row.get("probability_edge"), "expected_value": row.get("expected_value"), "entry_valid_until": now.isoformat(), "entry_snapshot_at": row.get("snapshot_at") or row.get("odds_captured_at") or now.isoformat()}
...
def send_alerts(root: Path, settings: BaseballSettings, now: datetime | None = None) -> int:
    ...
    for signal in pending:
        ...
        body = (f"<h2>⚾ UPLATI SADA</h2><p><b>{signal.get('match')}</b><br>{signal.get('league')}<br>"
                f"{signal.get('market_display')} @ {signal.get('odd')} · {signal.get('bookmaker')}</p>"
                f"<p>Model {float(signal.get('model_probability') or 0) * 100:.1f}% · Edge {float(signal.get('probability_edge') or 0) * 100:+.1f}pp · EV {float(signal.get('expected_value') or 0) * 100:+.1f}%</p>"
