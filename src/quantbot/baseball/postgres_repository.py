...
The module intentionally depends only on the DB-API connection protocol.  A
PostgreSQL driver (for example psycopg) is supplied by the application/runtime,
...
class PostgreSQLEvidenceRepository:
    ...
    def _observation_values(record: OddsObservation) -> tuple[Any, ...]:
        return (
            ...
            record.observed_at, record.retrieved_at, record.source_payload_ref,
            record.source_payload_checksum, record.schema_version, record.market_status,
    ...
    def _append(self, table: str, identity: str, values: tuple[Any, ...], columns: str,
                placeholders: str, canonical_index: int) -> bool:
        insert = (f"INSERT INTO {table} ({columns}) VALUES ({placeholders}) "
                  "ON CONFLICT DO NOTHING")
        compare = f"SELECT canonical_record FROM {table} WHERE " \
                  f"{('observation_id' if table == 'odds_observations' else 'pick_id')} = %s"
    ...
    def append_observation(self, record: OddsObservation) -> bool:
        columns = ("observation_id, game_id, market_family, line, selection, bookmaker, "
                   "decimal_odds, raw_price, observed_at, retrieved_at, source_payload_ref, "
                   "source_payload_checksum, schema_version, market_status, canonical_record")
        return self._append("odds_observations", record.observation_id,
                            self._observation_values(record), columns, ", ".join(["%s"] * 15), 14)
    ...
    def append_pick_event(self, record: PickEvent) -> bool:
        columns = ("pick_id, game_id, market_family, line, selection, decision, decision_reason, "
                   ...
                   "decision_decimal_odds, model_probability, fair_decimal_odds, "
                   "market_implied_probability, edge, expected_value_per_unit, uncertainty_metric, "
                   "source_data_cutoff_at, schema_version, canonical_record")
    ...
    def append_observations(self, records: Iterable[OddsObservation]) -> int:
        ...
        if not records:
            return 0
        try:
            inserted = sum(1 for record in records if self.append_observation(record))
            return inserted
        except Exception:
            self._connection.rollback()
            raise
    ...
    def get_observation(self, observation_id: str) -> OddsObservation | None:
        with self._connection.cursor() as cursor:
            ...
            row = cursor.fetchone()
        return None if row is None else _observation_from_dict(row[0])
    ...
    def get_pick_event(self, pick_id: str) -> PickEvent | None:
        with self._connection.cursor() as cursor:
            ...
            row = cursor.fetchone()
        return None if row is None else _pick_from_dict(row[0])
    ...
    def observations(self) -> tuple[OddsObservation, ...]:
        with self._connection.cursor() as cursor:
            ...
            return tuple(_observation_from_dict(row[0]) for row in cursor.fetchall())

    def pick_events(self) -> tuple[PickEvent, ...]:
        with self._connection.cursor() as cursor:
            cursor.execute("SELECT canonical_record FROM pick_events ORDER BY decision_at, pick_id")
            return tuple(_pick_from_dict(row[0]) for row in cursor.fetchall())
...
def _observation_from_dict(value: dict[str, Any]) -> OddsObservation:
    return OddsObservation(**value)


def _pick_from_dict(value: dict[str, Any]) -> PickEvent:
    return PickEvent(**value)
